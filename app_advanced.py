...
app.layout = html.Div([
    html.H2("TOEIC Mock (Reading) - Dash [Advanced]"),
    ...
    dcc.Store(id="ui-state", data={"busy": False, "lastError": None}),
    ...
    dcc.Loading(
        id="loading",
        type="circle",
        children=html.Div(id="question-area"),
    ),
    dcc.Interval(id="boot", interval=200, n_intervals=0, max_intervals=1),
    ...
    try:
        ...
        if engine == "openai":
            try:
                ...
                return no_update, no_update, {"busy": False, "lastError": f"LLM import error: {e}"}
        ...
        if not questions:
            ...
            return {"questions": fb_q}, {"index": 0, "score": 0, "answered": False}, {"busy": False, "lastError": "No question generated. Fallback used."}
        return {"questions": questions}, {"index": 0, "score": 0, "answered": False}, {"busy": False, "lastError": None}
    except Exception as e:
        print("Load error:", e)
        try:
            part_num = int(section_value) if section_value in (5, 6, 7) else 7
        ...
        fb = _fallback_dataset(part_num)
        fb_q = collect_questions(fb)
        return {"questions": fb_q}, {"index": 0, "score": 0, "answered": False}, {"busy": False, "lastError": str(e)}
...
def render_question(dataset, state, ui_state):
    if not dataset or not dataset.get("questions"):
        err = (ui_state or {}).get("lastError")
        msg = "データが未読み込みです。" if not err else f"生成に失敗しました: {err}"
        return html.Div(msg, style={"color": "#c00"}), "", ""
...
def set_busy(n_clicks, ui_state):
    st = dict(ui_state or {})
    st["busy"] = True
    st["lastError"] = None
    return st
...
def update_status(ui_state):
    busy = bool((ui_state or {}).get("busy"))
    err = (ui_state or {}).get("lastError")
    msg = "問題を生成しています…" if busy else (f"エラー: {err}" if err else "")
    return msg, busy, busy, busy, busy, busy, busy
...
def initial_autoload(n_intervals, engine, section_value, seed, p7_length, model):
